...
pid_t launch_svc(CONF *conf, const char *name)
{
    ...
    if ((dir = NCONF_get_string(conf, name, "dir")))
    {
        /* chroot into dir */
        if(chdir(dir))
            err(1,"error while chdir()");
        if(getuid()==0){
            if(chroot("."))
                err(1,"error while chroot()");
    ...
    if ((groups = NCONF_get_string(conf, name, "extra_gids")))
    {
        ...
        //ngids = getgroups(0,gids);
        if(setgroups(ngids,gids))
            err(1,"error while setgroups()");
        for (i = 0; i < ngids; i++)
